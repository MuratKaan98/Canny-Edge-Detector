...
class cannyEdgeDetector:
    ...
    def gaussian_kernel(self, size, sigma=1):   #Here gaussian kernel filter was created. Kernel was used for smoothing operation.
        size = int(size) // 2
    ...
    def sobel_filters(self, img):     #Sobel Filter design. For x and y dimension, two different filter was created.
        Kx = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], np.float32)
        Ky = np.array([[1, 2, 1], [0, 0, 0], [-1, -2, -1]], np.float32)

        Ix = ndimage.filters.convolve(img, Kx)   #Gradient calculation for X axis
        Iy = ndimage.filters.convolve(img, Ky)   #Gradient calculation for Y axis
